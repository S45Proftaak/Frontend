...
class GeneralLeaderboard extends Component {
  ...
  CycleLeaderboards(index) {
    switch (index) {
      ...
      case 1:
        this.GetScoreboardValues(
          "http://localhost:8020/scoreboard/get-scoreboard-most-eaten"
        );
        this.setState({
      ...
      <Container className="footer">
        ...
        <h5 className="text-center">
          <button
            ...
            onClick={() =>
              this.CycleLeaderboards(this.state.currentLeaderboard - 1)
            }
          >
            {"<"}
          </button>
              ...
              {this.state.fetchedData.map((res, id) => (
                <Col sm="4" key={id} className={"collum"}>
                  <div className="Combi">
                    ...
                    {res.totalPoints}
                    <br />
                    <div className={"rectangle rec" + id}>
                      {id + 1 === 1 ? id + 2 : id + 1 === 2 ? id : id + 1}
                    </div>
                  </div>
                </Col>
              ))}
